...
get - выполняет GET запрос на переданный в параметрах адрес и возвращает промис с объектом, который вернул сервер;

post - принимает объект с данными, которые будут переданы в JSON в теле запроса, и отправляет эти данные на адрес переданный как параметр при вызове метода. По умолчанию выполняется POST запрос, но метод запроса может быть переопределен заданием третьего параметра вызова.
...
toggleClass - Переключает CSS-класс на заданном элементе. Принимает HTMLElement, на котором переключается класс, имя класса для переключения и опционально булево значение принудительного управления;
setText - Устанавливает текстовое содержимое элемента. Принимает HTMLElement, в котором изменяется текст и значение, которое будет установлено;
setDisable - Изменяет статус блокировки элемента, делая его доступным или недоступным. Принимает HTMLElement, у которого изменяется атрибут disabled и булево значение, в которое нужно перевести статус блокировки;
setHidden - Скрывает элемент, устанавливая стиль display: none. Принимает HTMLElement, который нужно скрыть;
setVisible - Оторбажает элемент, удаляя стиль display. Принимает HTMLElement, который нужно отобразить;
setImage - Устанавливает изображение и альтернативный текст для элемента <img>. Принимает HTMLImageElement, в котором устанавливается изображение, путь к изображению и опционально альтернативный текст изображения;
render - Возвращает корневой DOM-элемент компонента. Опционально принимает объект данных для обновления состояния компонента. Возвращает HTMLElement.
...
emitChanges - Отправляет событие об изменении модели, позволяя другим компонентам отслеживать обновления данных.
...
addToBasket - добавить товар в корзниу.
...
counter - Устанавливает количество товаров в корзине, обновляя текст счетчика..
gallery - Заменяет текущие элементы каталога на новые, переданные в массиве.
locked - Устанавливает статус блокировки страницы, добавляя или удаляя CSS-класс на элементе _wrapper.
Класс Form:
...
id - Устанавливает или получает идентификатор товара.
title - Устанавливает или получает текст названия товара.
description - Устанавливает или получает текст описания товара.
price - Устанавливает или получает цену товара. Если цена не указана, отображает "Бесценно" и отключает кнопку.
image - Устанавливает изображение товара и задает альтернативный текст.
category - Устанавливает или поулчает текст категории товара.
button - Устанавливает текст кнопки.
Класс BasketCard:

Класс BasketCard расшеряет Card и используется для отображения карточки товара в корзине покупок, включая индекс товара, его название, цену и кнопку для удаления из корзины. Конструктор класса инициализирует основные элементы карточки (индекс, название, цена, кнопку удаления) и добавляет обработчик клика на кнопку удаления, если передано действие onClick.

Поля класса:

_email - Поле ввода для электронной почты.
_phone - Поле ввода для номера телефона.
Методы класса:
...
Класс наследует Component<ISuccess>. Он управляет отображением состояния успешного заказа. В конструкторе инициализирует компонент, устанавливает ссылки на элементы описания и кнопку закрытия. Также добавляет обработчик событий для кнопки закрытия.

Поля класса:

close - Элемент, отвечающий за закрытие модального окна.
description - Элемент, содержащий текстовое описание успешного завершения заказа.
Методы класса:

total - Устанавливает текст в элементе описания, отображая информацию о итоговой сумме заказа.
Класс Basket:
...
_list - Элемент, представляющий список товаров в корзине.
_total - Элемент, отображающий общую сумму товаров в корзине.
_button - Элемент кнопки, который инициирует процесс оформления заказа.
Методы класса:

items - Устанавливает элементы в корзину. Если корзина не пустая, заменяет содержимое списка на переданные элементы; если пустая, отображает сообщение о том, что в корзине нет товаров.
total - Устанавливает текст в элементе итоговой суммы.
Класс Card:
...
_index - HTML-элемент, в котором отображается индекс позиции товара в корзине.
_title - HTML-элемент для отображения названия товара.
deleteButton - Кнопка для удаления товара из корзины. При нажатии на нее срабатывает действие onClick, переданное в конструктор через объект actions.
Методы класса:
